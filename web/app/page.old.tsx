...
const useMockData = () => {
    ...
    return { bot, trades, orders, chart, setBot, setTrades, setOrders, resetForPairChange, updateSlippageForSize };
};

const formatCurrency = (n: number) => `$${n.toFixed(2)}`;

export default function Page() {
    const { bot, trades, orders, chart, setBot, setTrades, setOrders, resetForPairChange, updateSlippageForSize } = useMockData();
    ...
    const fetchRiskStatus = useCallback(async () => {
        try {
            ...
            if (data) {
                ...
                // Update position size from backend config
    ...
    // Initial data fetch - prioritize fast endpoints first
    useEffect(() => {
        // Fast API calls - bot internal state (no XRPL needed)
        fetchStatus();
        fetchRiskStatus();
        fetchTrades();

        // Slower calls - require XRPL connection, defer slightly
    ...
    // Set up polling intervals separately
    useEffect(() => {
        // Refresh risk status every 30 seconds (reduced from 10s)
        const riskInterval = setInterval(fetchRiskStatus, 30_000);
        // Refresh wallet info every 60 seconds (reduced from 30s)
        const walletInterval = setInterval(() => fetchWalletInfo(currentPair), 60_000);
        // Refresh orders every 30 seconds (reduced from 15s)
        const ordersInterval = setInterval(fetchOrders, 30_000);
        // Refresh trades every 30 seconds (reduced from 10s)
    ...
    }, [fetchRiskStatus, fetchWalletInfo, fetchOrders, fetchTrades, currentPair]);
    ...
    const applyTradingPair = async (pairKey: string) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                // Refresh orders to show any real open orders for this pair
                fetchOrders();
            } else {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-[#05080f] via-[#0b1221] to-[#090c14] text-slate-100">
            <div className="max-w-6xl mx-auto px-6 py-8 space-y-6">
                ...
                <div className="card bg-gradient-to-r from-slate-900/80 to-slate-800/50 border border-white/10 rounded-2xl px-5 py-4">
                ...
                <section className="grid lg:grid-cols-[1.7fr,1fr] gap-5 items-start">
                    <div className="card p-6 space-y-5">
                        <div className="flex items-center justify-between">
                            <div>
                                <div className="stat-label">XRP Price</div>
                                <div className="flex items-baseline gap-3">
                                    <span className="text-xl font-semibold">{currentPair?.key || 'Select pair'}</span>
                                    {currentPrice > 0 && (
                    ...
                    <div className="card p-6 space-y-5">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <div className="panel-header">Mode & guardrails</div>
                            </div>
                            <Pill icon={Shield} label={bot.paper ? 'PAPER' : 'LIVE'} tone={bot.paper ? 'neutral' : 'danger'} />
                        </div>
                        <div className="space-y-3 text-sm text-slate-300">
                            <div className="space-y-2 rounded-xl bg-white/5 px-4 py-3">
                            ...
                            <div className="flex items-center justify-between rounded-xl bg-white/5 px-4 py-3">
                                <span>Strategy</span>
                                <span className="text-slate-100 font-medium">{bot.strategy}</span>
                            </div>
                            <div className="flex items-center justify-between rounded-xl bg-white/5 px-4 py-3">
                                <span>Last Ledger</span>
                                <span className="text-slate-100 font-medium">{bot.lastLedger}</span>
                            </div>
                            <div className="flex items-center justify-between rounded-xl bg-white/5 px-4 py-3">
                                <span>Liquidity</span>
                                <span className="text-slate-100 font-medium">{bot.liquidity}</span>
                            </div>
                            <div className="flex items-center justify-between rounded-xl bg-white/5 px-4 py-3">
                                <span>Slippage</span>
                                <span className="text-slate-100 font-medium">{bot.slippageBps} bps</span>
                            </div>
                            <div className="flex items-center justify-between rounded-xl bg-white/5 px-4 py-3 gap-3">
                    ...
                    <div className="card p-6 space-y-5">
                        ...
                        <div className="space-y-3 max-h-80 overflow-y-auto">
                            {trades.length === 0 ? (
                                ...
                                <div key={t.id} className="flex items-center justify-between rounded-xl bg-white/5 px-4 py-3 text-sm">
                                    <div className="flex items-center gap-3">
                    ...
                    <div className="card p-6 space-y-5">
                        ...
                        <div className="space-y-3">
                            {orders.length === 0 ? (
                                ...
                                orders.map((o) => (
                                    <div key={o.id} className="flex items-center justify-between rounded-xl bg-white/5 px-4 py-3 text-sm">
                                        <div className="flex items-center gap-3">
                ...
                <section className="grid lg:grid-cols-2 gap-5">
                    <div className="card p-6 space-y-5">
                        <div className="flex items-center justify-between">
                            <div className="panel-header">Risk Dashboard</div>
                            <Badge tone={bot.risk.killSwitch ? 'danger' : 'neutral'}>{bot.risk.killSwitch ? 'Kill-switch ON' : 'Nominal'}</Badge>
                        </div>
                        <div className="grid grid-cols-2 gap-4 text-sm text-slate-300">
                            <div className="rounded-xl bg-white/5 px-4 py-3">Max Exposure <span className="block text-base text-slate-100 font-semibold mt-1">{bot.risk.maxExposure}</span></div>
                            <div className="rounded-xl bg-white/5 px-4 py-3">Current Exposure <span className="block text-base text-slate-100 font-semibold mt-1">{bot.risk.currentExposure}</span></div>
                            <div className="rounded-xl bg-white/5 px-4 py-3">Daily Loss Limit <span className="block text-base text-slate-100 font-semibold mt-1">{bot.risk.dailyLossLimit}</span></div>
                            <div className="rounded-xl bg-white/5 px-4 py-3">Kill Switch <span className="block text-base text-slate-100 font-semibold mt-1">{bot.risk.killSwitch ? 'ENGAGED' : 'OFF'}</span></div>
                        </div>
                    </div>
                    ...
                    <div className="card p-6 space-y-5">
                        ...
                        <div className="space-y-3 text-sm text-slate-300">
                            {[
                                ...
                                <div key={s.name} className="flex items-center justify-between rounded-xl bg-white/5 px-4 py-3">
                                    <div>
                ...
                {/* Trade Tape Section */}
                <section>
                    <TradeTape pairKey={selectedPairKey || undefined} maxRows={300} />
                </section>
            </div>
        </div>
    );
